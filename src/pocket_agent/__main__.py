"""Entry point: python -m pocket_agent"""
...
from pocket_agent.config.settings import Config, CloudTierConfig
from pocket_agent.core.loop import AgentLoop
from pocket_agent.persona import build_system_prompt, build_web_system_prompt
from pocket_agent.core.goal_aligner import GoalAligner
from pocket_agent.core.goals_paths import resolve_goals_path
from pocket_agent.inference.local import LocalBackend
from pocket_agent.memory.conversation import ConversationMemory
from pocket_agent.memory.structured import StructuredMemory
from pocket_agent.memory.plans import PlanMemory
from pocket_agent.tools.base import ToolRegistry
from pocket_agent.tools.web_search import WebSearchTool
from pocket_agent.tools.calendar import GoogleCalendarTool
from pocket_agent.tools.reminders import ReminderTool
from pocket_agent.tools.knowledge import KnowledgeTool
...
def main() -> None:
    ...
    log = logging.getLogger("pocket_agent")
    ...
    from pocket_agent.tools.files import FileTool
    ...
    if cfg.email.api_key:
        from pocket_agent.tools.email import EmailTool
    ...
    if cfg.atlassian.domain and cfg.atlassian.api_token:
        from pocket_agent.tools.jira import JiraTool, ConfluenceTool
    ...
    if atlassian_mcp_entry:
        from pocket_agent.mcp.client import MCPServerConfig, check_mcp_prerequisites
        from pocket_agent.mcp.gateway import MCPGatewayTool
    ...
    from pocket_agent.mcp.twelvewy import register_twelvewy, is_twelvewy_server
    ...
    from pocket_agent.core.alignment_judge import SemanticAlignmentJudge
    semantic_judge = (
    ...
    if cfg.engine.enabled and engine_llm:
        ...
        from pocket_agent.core.context import ContextManager
    ...
    if cfg.channel == "sms":
        from pocket_agent.channels.sms import SmsChannel

        SmsChannel(agent).run(async_init=_async_startup)
    else:
        try:
            from pocket_agent.channels.telegram import TelegramChannel
        except ImportError:
            log.error(
        ...
        if cfg.voice.enabled:
            from pocket_agent.voice.stt import create_stt
            ...
            if cfg.voice.tts_enabled:
                from pocket_agent.voice.tts import create_tts
        ...
        def _on_start():
            ...
            if cfg.digest.enabled and cfg.telegram.allowed_users:
                from pocket_agent.core.digest import DigestService
            ...
            if cfg.monitor.enabled and cfg.telegram.allowed_users:
                from pocket_agent.core.monitor import MonitorService
                ...
                if cursor_manager and jira_tool:
                    from pocket_agent.cursor.jira_bridge import JiraCursorBridge
            ...
            if cfg.sms.enabled and (
                ...
                from pocket_agent.channels.sms_listener import SmsListener
            ...
            if cfg.web.enabled:
                ...
                if web_llm:
                    from pocket_agent.web.agent import WebAgent
                    from pocket_agent.web.ratelimit import RateLimiter
                    from pocket_agent.web.app import create_app
                    ...
                    lead_outreach = None
                    if email_tool:
                        from pocket_agent.web.outreach import LeadOutreach
